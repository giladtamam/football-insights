...
export function useExport() {
  ...
  const exportToCSV = useCallback(
    ({ headers, rows, filename }: ExportableData) => {
      const csvContent = [
        headers.join(","),
        ...rows.map((row) =>
          row
  ...
  const copyToClipboard = useCallback(
    async ({ headers, rows }: ExportableData) => {
      const textContent = [
        headers.join("\t"),
        ...rows.map((row) =>
  ...
  const exportFixtures = useCallback(
    (fixtures: any[], filename = "fixtures") => {
      const headers = [
        "Date",
        "Time",
        "Home Team",
  ...
  const exportStandings = useCallback(
    (standings: any[], leagueName: string) => {
      const headers = [
        "Rank",
        "Team",
        "Played",
      ...
      return {
        headers,
        rows,
        filename: `${leagueName.toLowerCase().replace(/\s+/g, "-")}-standings`,
      };
    },
    []
  );
  ...
  const exportOdds = useCallback((oddsData: any[], filename = "odds") => {
    ...
    const rows = oddsData.map((o) => [
      ...
      o.impliedProbabilities?.home
        ? (o.impliedProbabilities.home * 100).toFixed(1)
        : "",
      o.impliedProbabilities?.draw
        ? (o.impliedProbabilities.draw * 100).toFixed(1)
        : "",
      o.impliedProbabilities?.away
        ? (o.impliedProbabilities.away * 100).toFixed(1)
        : "",
  ...
  const exportMatchAnalysis = useCallback((fixture: any) => {
    ...
    const rows = [
      ...
      [
        "Goals",
        fixture.goalsHome?.toString() || "",
        fixture.goalsAway?.toString() || "",
      ],
      [
        "xG",
    ...
    return {
      headers,
      rows,
      filename: `${fixture.homeTeam?.name || "home"}-vs-${
